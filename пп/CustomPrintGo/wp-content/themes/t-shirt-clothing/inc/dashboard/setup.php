...
<div class="theme-offer">
  ...
  if(isset($_GET['import-demo'])){
    ...
    if (!$menu_exists) {
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'  => __('Home', 't-shirt-clothing'),
      ...
      $parent_page_item_id = wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'  => __('Pages', 't-shirt-clothing'),
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'   => __('Page Left Sidebar', 't-shirt-clothing'),
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'   => __('Page Right Sidebar', 't-shirt-clothing'),
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'  => __('Shop', 't-shirt-clothing'),
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'  => __('Categories', 't-shirt-clothing'),
      ...
      wp_update_nav_menu_item($menu_id, 0, array(
          'menu-item-title'  => __('Contact Us', 't-shirt-clothing'),
    ...
    for($i=1;$i<=4;$i++){
      ...
      $clothing_store_post_image_url = get_stylesheet_directory_uri().'/assets/images/slide.png';

      $clothing_store_image_name = 'slide.png';
    ...
    set_theme_mod('t_shirt_clothing_product_category_title','Find with category');

    set_theme_mod('t_shirt_clothing_product_category_subtitle','Upto 60% off on Handloom clothing & furnishings');

    $post_cat_category = wp_create_category('latest'); 

    $post_cat_title=array('Sports Wear',"Kid's Fashion",'Office Wear');

    set_theme_mod( 't_shirt_clothing_cat_count', '3' );

    for($i=1;$i<=3;$i++){

      $title = $post_cat_title[$i-1];
      $content = 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.';

      // Create post object
      $t_shirt_clothing_my_post = array(
       'post_title'    => wp_strip_all_tags( $title ),
       'post_content'  => $content,
       'post_status'   => 'publish',
       'post_type'     => 'post',
       'post_category' => array($post_cat_category),
       'order'         => 'ASC'
      );

      $t_shirt_clothing_cat_post_id = wp_insert_post($t_shirt_clothing_my_post);

      $t_shirt_clothing_cat_post_image_url = get_stylesheet_directory_uri().'/assets/images/image'.$i.'.jpg';

      $t_shirt_clothing_cat_image_name = 'image'.$i.'.jpg';
      $t_shirt_clothing_cat_upload_dir       = wp_upload_dir(); 
      // Set upload folder
      $t_shirt_clothing_cat_image_data       = file_get_contents($t_shirt_clothing_cat_post_image_url); 
       
      // Get image data
      $t_shirt_clothing_cat_unique_file_name = wp_unique_filename( $t_shirt_clothing_cat_upload_dir['path'], $t_shirt_clothing_cat_image_name ); 
      // Generate unique name
      $filename= basename( $t_shirt_clothing_cat_unique_file_name ); 
      // Create image file name
      // Check folder permission and define file location
      if( wp_mkdir_p( $t_shirt_clothing_cat_upload_dir['path'] ) ) {
          $file = $t_shirt_clothing_cat_upload_dir['path'] . '/' . $filename;
      } else {
          $file = $t_shirt_clothing_cat_upload_dir['basedir'] . '/' . $filename;
      }
      file_put_contents( $file, $t_shirt_clothing_cat_image_data );
      $wp_filetype = wp_check_filetype( $filename, null );
      $t_shirt_clothing_cat_attachment = array(
          'post_mime_type' => $wp_filetype['type'],
          'post_title'     => sanitize_file_name( $filename ),
          'post_content'   => '',
          'post_type'     => 'post',
      ...
      $attach_id = wp_insert_attachment( $t_shirt_clothing_cat_attachment, $file, $t_shirt_clothing_cat_post_id );
      require_once(ABSPATH . 'wp-admin/includes/image.php');
      $attach_data = wp_generate_attachment_metadata( $attach_id, $file );
          wp_update_attachment_metadata( $attach_id, $attach_data );
          set_post_thumbnail( $t_shirt_clothing_cat_post_id, $attach_id );

    }

    set_theme_mod( 't_shirt_clothing_category_setting', 'latest' );
  ...
  <p class="plugin-text"><?php echo esc_html_e('Before Demo Import first install given plugin, ','t-shirt-clothing'); ?><span><?php echo esc_html_e('WooCommerce','t-shirt-clothing'); ?></span></p>
  <p class="note"><?php esc_html_e("If your website is already live and containing data, please make a backup.This importer will override the T-Shirt Clothing's new customizable values.",'t-shirt-clothing'); ?></p>
  <form id="mep-demo-importer-form" action="<?php echo esc_url(home_url()); ?>/wp-admin/themes.php?page=clothing-store-guide-page" method="POST">
    <input type="submit" name="submit" value="<?php echo esc_attr( __('Begin With Demo Import', 't-shirt-clothing') ); ?>" class="button button-primary button-large">
    <a href="<?php echo esc_url(home_url('/')); ?>" target="_blank" class="button button-primary button-large"><?php esc_html_e('View Site','t-shirt-clothing'); ?></a>
  </form>
  <div class="mep-spinner-div"><p class="spinner"></p></div>
  <div class="success">
    <?php if (isset($_GET['import-demo'])) {
       echo esc_html(__('Demo Import Successful', 't-shirt-clothing'));
    } ?>
  </div>
