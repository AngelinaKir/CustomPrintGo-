...
// Date formatting
function clothing_store_display_shop_date() {
    // Get the date type option
    $clothing_store_date_type = get_theme_mod( 'clothing_store_date_type', 'published' );

    // Determine the date to display based on the type
    if ( $clothing_store_date_type === 'modified' && get_the_modified_time( 'U' ) !== get_the_time( 'U' ) ) {
        $clothing_store_date_to_display = get_the_modified_date( get_option( 'date_format' ) );
    } else {
        $clothing_store_date_to_display = get_the_date( get_option( 'date_format' ) );
    }

    // Output the date HTML

    echo esc_html( $clothing_store_date_to_display );
}
...
function clothing_store_setup() {
	...
	load_theme_textdomain( 'clothing-store', get_template_directory() . '/languages' );
